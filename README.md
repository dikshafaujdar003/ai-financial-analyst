...
Why build ReAct from scratch? LangChain's create_react_agent was removed in v1.2+, migrated to LangGraph. Building from scratch demonstrates understanding of the underlying Thought → Action → Observation loop rather than just using a black-box framework.

Why Groq? Free tier with fast inference on Llama 4, no credit card required. Easily swappable with OpenAI or Gemini by changing 2 lines in my_agent.py.

Why multi-agent? Each specialist has a focused system prompt and limited tool set, which reduces hallucination compared to a single agent with all tools.
